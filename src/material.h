...
public:
	...
	__device__ __host__ Material(bool isEmissive = false, texture * tex = new solid_color(color(0, 0, 0)), float specularRate = 0.0f, float roughness = 0.0f, float refractRate = 0.0f, float indexOfRefraction = 1.0f, float refract_roughness = 0.0f)
		: isEmissive(isEmissive), matTexture(tex), specularRate(specularRate), roughness(roughness), refractRate(refractRate), indexOfRefraction(indexOfRefraction), refract_roughness(refract_roughness) {
	}

	__device__ __host__ Material(bool isEmissive = false, color materialColor = color(0, 0, 0), float specularRate = 0.0f, float roughness = 0.0f, float refractRate = 0.0f, float indexOfRefraction = 1.0f, float refract_roughness = 0.0f)
		: Material(isEmissive, new solid_color(materialColor), specularRate, roughness, refractRate, indexOfRefraction, refract_roughness) {
	}
