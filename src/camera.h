...
public:
    ...
    vec3 originalUpVector = vec3(0,0,0);
    ...
    __device__ camera_device()
        ...
        pixelSamplesScale(1.0), upper_clamp(1000)
    {
    }
    ...
    __host__ void updateFOV(float fov)
    {
        verticalFov = fov;
    }

    __host__ void doCameraMovement(float right, float up, float forward)
    {
        vec3 displacement = right * u + up * v + forward * w;
        lookFrom += displacement;
        lookAt += displacement;
    }

    __host__ void moveWorldUpVector(float rotation)
    {
        worldUpVector = rotate(w, rotation * 3.14159265f / 180.0f, worldUpVector);
    }

    __host__ void setWorldUpVector(const vec3& newUp)
    {
        worldUpVector = newUp;
    }
    ...
    {
        //initialize original world up vec
        if(originalUpVector.length() == 0)
            originalUpVector = worldUpVector;
    ...
    __device__ color ray_color(const ray & r_in, int max_depth, const hittable & world, const hittable & lights, BVHNode & search_tree) {
        ...
        for (int depth = max_depth; depth > 0; depth--) {
            ...
            if (!rec.mat->scatter(cur_ray, rec, scatter_attenuation, scattered, p_rec)) {
                final_color += attenuation * scatter_attenuation;
                return final_color;
            }
